...
def make_release_body(version, image_name, release_info):
    ...
    github_common.add_output("PR_release_body",{body})
...
def main():
    ...
    if args.api_url:
        ...
        github_common.add_output("PR_tarball_name", {asset_file})
        if check_if_only_version_file_is_modified(args.api_url):
            ...
            github_common.add_output("PR_version",{version_info["version"]})
            github_common.add_output("PR_release_image",{version_info["quay-image"]})
            github_common.add_output("PR_release_info",{version_info["release-info"]})
            github_common.add_output("PR_includes_release","true")
        ...
        if args.version:
            ...
            if version_compare > 0 :
                ...
                github_common.add_output("updated","true")
